...
from config import (
    MT5_PATH, BARS, INITIAL_BALANCE,
    OPTIMIZER, SEARCH_SPACE, GA_CONFIG, MC_CONFIG
)
...
def run_backtest_and_plot(params, df, initial_balance, strategy_name):
    ...
    plt.title(f"Equity Curve ({strategy_name}) - Best GA Result")
    plt.xlabel("Step #")
    plt.ylabel("Balance")
    plt.grid(True)
    plt.legend()
...
def main():
    ...
    # ======================
    if OPTIMIZER == "none":
        params = {"ma_fast": 10, "ma_slow": 40, "atr_mul": 1.5, "risk_per_trade": 0.01}
